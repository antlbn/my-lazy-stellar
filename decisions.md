...
Решение: внешние возможности доступны через ports: `LLMProvider`, `SearchProvider`, `WeatherProvider`, `SkyCalendarProvider`, `SessionStore`.

Причина: даже если PydanticAI предоставляет готовые tools, выбор поисковика, weather API или календарного источника остается инфраструктурной деталью.
...
Причина: объем данных небольшой, а ранжирование требует объяснения trade-offs: доступность, погода, безопасность, видимые объекты, личные ограничения.
...
## D008. Sky calendar как отдельная возможность

Статус: принято.

Решение: учет того, что сейчас видно на небе, выделяется в отдельный `SkyCalendarProvider`.

Причина: это не просто phrasing в prompt, а отдельный источник данных/правил, который влияет на рекомендацию.

Последствия:

- MVP должен добавить модель `SkyCalendarReport`;
- реализация может стартовать со статических правил или простого API;
- финальный ответ должен упоминать релевантные видимые объекты.
...
- custom spans нужны вокруг `ChatUseCase`, search/weather/calendar adapters и session store;
